...
- Plain tool use: every row flowed through the context, 9,451 billed input tokens, and the model, summing 240 rows in its head, failed to produce a clean answer.
- Programmatic: the rows went to the sandbox, 6,828 billed input tokens (about 28% fewer), and the sandbox code computed the exact winner correctly.

One honest scope line so the number holds when you run it: the win is fan-out shaped. On a task that is just one tool call the docs note it is flat to about 8% more expensive, so this is for tool-heavy agents that fan out, not every agent. On a real slow tool you also save the per-call model round-trip. It is GA, but not on Bedrock or Vertex and not ZDR-eligible, so check your stack.
