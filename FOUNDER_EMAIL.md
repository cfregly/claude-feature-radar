...
You do not have to take the 28% on faith. The programmatic-tool-calling edge ships as a forkable app. Clone the repo, paste your own Messages-API tool dict and the Python that runs it into one file (app/my_tool.py), and run make app. It runs your own fan-out task twice, plain tool use vs programmatic, and prints your before/after input-token bill and the dollar delta at the model's price, with an upfront cost line before it spends anything. Out of the box it ships a worked example, so make app-check gives you a real number before you change a line. The whole thing costs $0.08 on Sonnet to reproduce.

{repo_url}
(The repository link resolves on publish.)

Run it yourself: make setup, then cp .env.example .env and paste your Anthropic key, then make ptc. $0.08 on Sonnet for the two runs, every token read off the real API.
