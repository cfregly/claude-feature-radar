...
regenerated by `make programmatic-tool-calling-cache-context`.

```bash
make programmatic-tool-calling                # estimated $0.08 token/API cost on Sonnet, needs ANTHROPIC_API_KEY
make programmatic-tool-calling-cache-context  # deterministic programmatic tool calling + cache + 1M cost model, $0
...
make programmatic-tool-calling-cache-context  # deterministic combined cost receipt, $0
```
