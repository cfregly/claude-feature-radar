...
An operator-owned engine that repeatedly finds live Claude platform feature edges, proves them on
real workloads, and publishes only verified Claude wins into public runnable briefs.

Package role: this is the method repo. It is useful for operators and reviewers, but the
founder-facing output is `claude-feature-hits`. Product-owner feedback lives in the misses checkout.
...
Routing rule: `claude-ahead` plus public value goes to `feature-hits`. Add a misses mirror when the
win has a product-owner caveat worth acting on. Measured or claim-relevant `parity`, any
`claude-behind`, stale claims, and useful held public framings go to `feature-misses`.
`never-evaluated` stays radar-only unless it blocks an existing public claim or has a product-owner
ask. A held candidate stays radar-only unless the hold itself is useful roadmap signal.
...
Production blocker intake lives in [`blockers/`](blockers/). Each JSON record is anonymized field
signal that renders into a product-owner misses `GAP_PACKET.md`: replayable case, business
consequence, priority argument, receiving owner, workaround or fix state, founder follow-up, and the
learning update. `make blockers` writes packets. `make check-blockers` verifies the configured
misses checkout still matches radar.

Managed Agents operations comparison also lives here, not in the standalone deep-dive repo.
Run `make compare-deps` once. Then `make managed-agents-ops` or
`python run.py managed-agents-ops` runs the canonical ops-triage workload across Claude Managed
Agents, a self-managed Claude Messages tool loop, OpenAI Agents SDK, and Google ADK with Gemini. It
writes the standard radar receipt to ignored `data/last_managed_agents_operations.json`. Current
status: mechanically vetted on the ops-triage workload, but not ahead. The receipt records the
Operations scorecard directly: orchestration lines removed, state or cleanup components avoided,
failed-run teardown success, interruption resume behavior, time to first correct agent, and token/API
cost from session usage when the API exposes it. Managed Agents stays a held Operations candidate
until that receipt shows same correctness, materially less glue code or fewer failure modes, and no
worse token/API cost against the self-managed loop.

Other Operations-shaped candidates are deliberately not promoted yet. Programmatic tool calling has a
formal cost receipt, but not an app-code or runbook-removal receipt. The citation and grounding edges
count hosted objects and setup calls, but they are still accuracy or data-locality claims unless a
separate scorecard quantifies app operations removed. Task budgets and fallback credit need repeated
tool-loop failure, handoff, retry, and cleanup measurements before they can be public Operations wins.
Code execution state and retention-resume prove state behavior, not a maintenance win by themselves.
...
## Current confirmed public lead: programmatic tool calling, 74% fewer billed input tokens than the same Claude agent without the feature (`make programmatic-tool-calling`)

Current adversarial status, 2026-06-24: confirmed. Claude Opus 4.8 xhigh and GPT-5.5 xhigh both let
the measured fan-out framing survive: the task shows a real token-cost win with a same-answer gate, and the
...
If your agent decides which customers are at risk, it may need support tickets, product logs, usage
metering, CRM notes, and compliance docs. Direct tool use sends every raw row into the model context.
Programmatic tool calling lets Claude write code in a sandbox that calls your tool, rejects malformed
rows, joins evidence by account, and passes just the compact decision packet back to the model.

Claude has a feature for this, no beta header. Add `allowed_callers: ["code_execution_20260120"]` to a
tool and include the code execution tool. The bulky outputs go to the sandbox, not the model.

Measured on the same fan-out task two ways, same model (Sonnet 4.6), same expected accounts required:
five evidence sources, 600 raw customer-evidence rows, and three at-risk accounts.

| mode | billed input tokens | what happens to the raw evidence rows |
|---|--:|:--|
| plain tool use | 54,989 | all rows flow through the model context |
| programmatic tool calling | 14,299 | the sandbox reduces, only the compact packet returns |

That is 74% fewer billed input tokens against the same Claude agent without programmatic tool calling.
Both paths return `acct_1842`, `acct_2199`, and `acct_7731`. Cost scope: this is token/API cost, not
all-in production COGS. Code execution runtime can bill separately after the monthly free allowance.
The full saved output is [`edges/programmatic-tool-calling/sample.txt`](edges/programmatic-tool-calling/sample.txt).

The expanded programmatic tool calling + prompt-cache + 1M-context proof is deterministic arithmetic
over dated prices plus that saved programmatic tool calling output. On a 700,000-token stable-prefix
session with 100 turns and 200,000 raw tool-output tokens per turn, Claude cache + programmatic tool
calling is `$26.04` vs `$85.44` for Claude cache without programmatic tool calling, `$135.55` for
best-case GPT-5.5 1M+cache without programmatic tool calling, and `$40.67` for Gemini 3.5 Flash
cache+1M without programmatic tool calling. That is 80.8% lower than the OpenAI modeled stack and
36.0% lower than the Gemini modeled stack. The cliff is the programmatic tool calling leg: cache and
1M context do not remove fresh tool-output tokens if the raw outputs keep returning to model context.
Those are model-token estimates and exclude any separate code-execution runtime charge.
The measured output is [`edges/programmatic-tool-calling-cache-context/receipt.json`](edges/programmatic-tool-calling-cache-context/receipt.json),
regenerated by `make programmatic_tool_calling_cache_context`.

```bash
make programmatic-tool-calling                # estimated $0.08 token/API cost on Sonnet, needs ANTHROPIC_API_KEY
make programmatic_tool_calling_cache_context  # deterministic programmatic tool calling + cache + 1M cost model, $0
make publish-hits                             # regenerate the public feature-hits code bundle from radar
```

Reproduce it on your own tool in the public bundle: edit
[`engine/public_hits_bundle/programmatic_tool_calling/founder_workload.py`](engine/public_hits_bundle/programmatic_tool_calling/founder_workload.py),
paste your Messages-API tool dict and the Python that runs it, then `make publish-hits` updates the
public checkout. In the public repo, `make programmatic_tool_calling` runs the same fan-out task twice
over your tool and prints your own before-and-after billed-input table and dollar delta.
...
- The programmatic tool calling cookbook (runnable): https://platform.claude.com/cookbook/tool-use-programmatic-tool-calling-programmatic_tool_calling
...
make programmatic_tool_calling_cache_context  # deterministic combined cost receipt, $0
```
...
```
engine/public_hits_bundle/  the canonical public feature-hits code bundle, including the programmatic tool calling edit surface
