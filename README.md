...
## Current confirmed public lead: programmatic tool calling, about 27% fewer billed input tokens than the same Claude agent without the feature (`make programmatic-tool-calling`)
...
|---|--:|:--|
| plain tool use | 9,494 | all 240 outputs flow through the model's context |
| programmatic | 6,910 | the sandbox aggregates, only the answer (east) reaches the model |

A 27% input-token cut against the same Claude agent without programmatic tool calling, and the sandbox
...
that saved PTC output. On a 700,000-token stable-prefix session with 100 turns and 200,000 raw
tool-output tokens per turn, Claude cache+PTC is `$26.04` vs `$85.44` for Claude cache without PTC,
`$135.55` for best-case GPT-5.5 1M+cache without PTC, and `$40.67` for Gemini 3.5 Flash
cache+1M without PTC. That is 80.8% lower than the OpenAI modeled stack and 36.0% lower than the
Gemini modeled stack. The cliff is the PTC leg: cache and 1M context do not remove fresh tool-output
tokens if the raw outputs keep returning to model context. Those are model-token estimates and
exclude any separate code-execution runtime charge.
The measured output is [`edges/programmatic-tool-calling/ptc-cache-context.json`](edges/programmatic-tool-calling/ptc-cache-context.json),
