...
| Edge | What you get | Measured output | Run |
|---|---|---|---|
| Programmatic tool calling | The model writes one sandbox script that calls your tool in a loop and returns only the answer, so the bulky outputs never hit the context. | 54,989 to 14,299 billed input tokens, about 74% fewer than the same Claude agent without the feature, same answer | `make programmatic-tool-calling` |
