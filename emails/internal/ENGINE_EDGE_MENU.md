...
| Edge | What you get | Measured output | Run |
|---|---|---|---|
| Programmatic tool calling | The model writes one sandbox script that calls your tool in a loop and returns only the answer, so the bulky outputs never hit the context. | 9,494 to 6,910 billed input tokens, about 27% fewer than the same Claude agent without the feature, same answer | `make programmatic-tool-calling` |
