# Founder email draft (cadence, cache_diagnostics, cost)
...
Claude can name the cache-miss cause on a request where competitor usage objects expose counters but no typed root-cause field.

Here is the receipt path: Estimated check cost: $0.14 using your own API key (estimated 0.3 minutes). Clone the repo and run one command:
...
- edge key: cache_diagnostics
- axis: cost
- verdict: claude-ahead
- reproduce: make cache_diagnostics (estimated $0.14)
