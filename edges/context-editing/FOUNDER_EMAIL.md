...
And the part I will not hide: this is the one edge where a competitor is close. OpenAI ships
server-side compaction that bounds context too (it summarizes the old turns rather than clearing them
in place), and on a job at moderate scale all three big platforms finish, our cross-vendor run is a
