...
DIFFERENTIATORS = [
    {
        ...
        "why": "No named OpenAI or Google equivalent keeps a developer's own custom-tool OUTPUTS out "
               ...
               "programmatic-tool-calling): on a 600-row customer-evidence fan-out task billed input fell from 54,989 to 14,299 tokens, "
               "74% fewer billed input tokens (Anthropic's own doc reports about 24%) on a same-answer run. Fan-out-shaped "
    ...
    {
        "key": "citations", "axis": "accuracy", "rank": 2,
        ...
        "why": "The wedge is convenience and the no-store bundle, NOT an accuracy guarantee over a "
    ...
    {
        "key": "pdf-citations", "axis": "accuracy", "rank": 5,
    ...
    {
        "key": "search-results", "axis": "accuracy", "rank": 6,
    ...
    {
        "key": "grounding-stack", "axis": "accuracy", "rank": 7,
    ...
    {
        "key": "web-citations", "axis": "accuracy", "rank": 8,
    ...
    },
    {
        ...
        "why": 'All three frontier arms returned the exact list on the headline run (saved output edges/exact-list-ledger/receipt.json, 2026-06-19), so the edge is NOT exclusivity. The win is cost and wall-clock time at equal accuracy: Claude $0.6700/60.7s vs OpenAI gpt-5.5 compaction $1.8425/164.3s (about 64% cheaper, 63% faster) vs Gemini 3.1 Pro full context $2.5690/201.0s (about 74% cheaper, 70% faster). A 5-run repeat confirmed 5/5 exact on both server-side-managed arms (Claude and OpenAI), so accuracy is not a fluke. The cost margin is config-sensitive: about 65% cheaper at keep=1, about 8% at keep=3. promotable_edge=true in the saved output.',
...
CHOSEN = (
    ...
    "GA, no named competitor equivalent. Measured (make programmatic-tool-calling): billed input fell 74% on a 600-row "
    "customer-evidence fan-out task (Anthropic's doc reports about 24%) on a same-answer run. Fan-out-shaped, it adds "
