...
the rates in model_catalog.py. Code execution runtime billing is tracked separately from token usage, so
...
from .model_catalog import get

WEB_SEARCH_PER_REQUEST = 10.0 / 1000.0
BATCH_DISCOUNT = 0.5

# Code execution runtime is not a token/API usage bucket. The Messages response tracks
# code_execution_requests, while the billable runtime meter has its own free allowance and floor.
CODE_EXECUTION_HOURLY_USD = 0.05
CODE_EXECUTION_MIN_MINUTES = 5
CODE_EXECUTION_FREE_HOURS_PER_ORG_MONTH = 1_550
...
def code_execution_runtime_floor_usd(containers: int = 1) -> float:
    """Post-free-allowance runtime floor exposure for code execution.

    The API usage object does not report exact runtime seconds. Current pricing bills code execution
    separately from token usage when it is not paired with web search or web fetch, with a five-minute
    minimum per container after the monthly free allowance. This helper is a lower-bound exposure
    estimate for one or more newly billed containers, not an exact invoice calculation.
    """
    return containers * CODE_EXECUTION_HOURLY_USD * CODE_EXECUTION_MIN_MINUTES / 60.0
...
def cost_from_buckets(model: str, *, fresh_input: int, cached: int, output: int) -> float:
    ...
    competitor price lives in exactly one place, common/model_catalog.py.
    """
    m = get(model)
    return (
