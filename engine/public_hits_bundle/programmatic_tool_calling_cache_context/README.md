...
![demo](https://raw.githubusercontent.com/cfregly/claude-feature-hits/main/programmatic_tool_calling_cache_context/demo.gif)

[![Claude proof: 36% lower than Gemini 3.5 Flash cache+1M without programmatic tool calling](https://img.shields.io/badge/Claude%20proof-36%25%20lower%20than%20Gemini%203.5%20Flash%20cache%2B1M-2F855A)](https://github.com/cfregly/claude-feature-hits/blob/main/programmatic_tool_calling_cache_context/sample.txt)
...
the Gemini row. The command validates the committed 54,989 -> 14,299 programmatic-tool-calling receipt, then
applies the checked price table to the larger workload.
...
## Run it ($0, no API call)

```
make programmatic_tool_calling_cache_context
```

`make programmatic_tool_calling_cache_context` reads `programmatic_tool_calling/sample.txt`, checks that
`receipt.json` cites the same 54,989 -> 14,299 basis, and prints the larger cache plus 1M-context cost
table. It does not call a model. The table excludes any separate code-execution runtime charge after
the free allowance.

## Why this is separate from plain programmatic tool calling

`programmatic_tool_calling` proves the live mechanism on a real fan-out task. This artifact adds the
